...
import { apiErrorMessage, ticTacToeGamePath } from "../../utility/ticTacToe";
import useButtonColorFix from "../../utility/useButtonColorFix";

const NOTICE_MS = 3800;
const onGameScreen = () => window.location.pathname.startsWith("/study/games/tic-tac-toe/");
...
function RequestCard({ request, busy, onAccept, onDecline, onExpired }) {
  ...
  return (
    <Motion.div
      ...
      role="alertdialog"
      aria-label={`${name} ${rematch ? "wants to play again" : "invited you to play Tic-Tac-Toe"}`}
      data-testid="ttt-request"
      ...
      <div className="flex items-center gap-3 p-3.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs leading-snug text-muted">
            {rematch ? "wants to play again" : "invited you to play Tic-Tac-Toe"}
          </p>
        </div>
        <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-soft text-sm font-black text-accent" aria-hidden="true">
          ✕○
        </span>
      </div>
...
/**
 * Application-level Tic-Tac-Toe popup: invitations and rematch requests with
 * Accept / Decline, plus short status notices. Mounted once, above both the
 * main and the Study shells (see Router/Main.jsx), so it appears on ANY page —
 * Feed, Profile, Messages, Study, Quiz, Games… Nothing here decides anything:
 * requests come from the server (over the socket, and re-fetched on reconnect),
 * and Accept/Decline are server calls.
 */
export default function TicTacToeGlobalHost({ user }) {
  ...
  const loadPending = useCallback(async () => {
    try {
      const { data } = await api.get("/games/tic-tac-toe/invites/pending");
  ...
  useRealtime("ticTacToe:invite", onIncoming);
  useRealtime("ticTacToe:rematch", onIncoming);

  const onClosed = (event) => remove(event.detail?.requestId || event.detail?.inviteId);
  useRealtime("ticTacToe:invite:cancelled", onClosed);
  useRealtime("ticTacToe:invite:expired", onClosed);
  useRealtime("ticTacToe:rematch:cancelled", onClosed);
  useRealtime("ticTacToe:rematch:expired", onClosed);

  // Someone (maybe another of my tabs) accepted: close the popup; if the
  // OTHER player accepted my request, take me straight into the game.
  const onAccepted = (event) => {
    const { requestId, gameId, by } = event.detail || {};
    remove(requestId);
    if (by && by !== user.id && gameId) {
      notify("Your friend accepted — game on!", "success");
      navigate(ticTacToeGamePath(gameId));
    }
  };
  useRealtime("ticTacToe:invite:accepted", onAccepted);
  useRealtime("ticTacToe:rematch:accepted", onAccepted);

  const onDeclined = (event) => {
    const { requestId, by, kind } = event.detail || {};
    remove(requestId);
    if (by && by !== user.id) {
      // On the game screen the "Rematch declined." message is shown in place.
      if (kind === "rematch" && onGameScreen()) return;
      notify(kind === "rematch" ? "Rematch declined." : "Your invitation was declined.", "info");
    }
  };
  useRealtime("ticTacToe:invite:declined", onDeclined);
  useRealtime("ticTacToe:rematch:declined", onDeclined);

  // Messages raised by other parts of the app (e.g. "Invitation sent").
  useRealtime("ticTacToe:notice", (event) => notify(event.detail?.message, event.detail?.tone));
  ...
  const respond = async (request, action) => {
    ...
    try {
      const { data } = await api.post(`/games/tic-tac-toe/${base}/${request.id}/${action}`);
      remove(request.id);
      if (action === "accept") navigate(ticTacToeGamePath(data.data.game.id));
    } catch (error) {
      // Expired / withdrawn / no longer friends: say so and clear the popup.
      remove(request.id);
  ...
  return (
    <div
      aria-live="polite"
      className="pointer-events-none fixed inset-x-0 top-[calc(var(--topbar-h,72px)+8px)] z-[80] flex flex-col items-center gap-2 px-3 sm:items-end sm:pr-4"
    >
