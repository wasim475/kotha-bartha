 ...
 */
export default function AdminStatCard({ label, value, hint, icon, loading, tone }) {
  return (
    <Card className="flex min-w-0 items-start gap-3" data-testid="admin-stat" data-loading={loading ? "true" : "false"}>
      ...
      <div className="min-w-0 flex-1">
        ...
        {loading ? (
          ...
          <p className={`mt-0.5 font-display text-2xl font-semibold tabular-nums ${tone || "text-ink"}`}>{Number(value ?? 0).toLocaleString()}</p>
        )}
